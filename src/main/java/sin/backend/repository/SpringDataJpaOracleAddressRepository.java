...
public interface SpringDataJpaOracleAddressRepository extends JpaRepository<Address, Long> {
    ...
    List<Address>findByName(String name);// 테이블 컬럼에 의존적인 select는 직접 만들어줘야 한다
    //JPQL -> select a from Address a where a.name=:name // :name은 넘어온 파라미터의 name
    List<Address>findByNameAndAddr(String name, String addr);// findByNameAndAddr이름이 엄청 중요하다 And, Or, ...등등 바꿔 쓸 수 있다

    List<Address> findByNameContaining(String name);// XXXContaining()은 like(%신%) 연산자 역할
